...
@section('content')
<form id="formPost" action="{!! route('backend.post.update', [$postInfo->id]) !!}" method="POST" novalidate>
    {{ csrf_field() }}
    <div class="form-row justify-content-center">
        <div class="col-12 col-lg-6">
            <div class="form-group">
                <label for="title" class="form-label form-required">Title</label>
                <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $postInfo->title) }}" placeholder="Title" data-code="true" data-for="#code" data-link="{!! route('backend.create-code') !!}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="description" name="description" placeholder="Description">{{ old('description', $postInfo->description) }}</textarea>
                {!! show_error($errors, 'description') !!}
            </div>
            <div class="form-group">
                <label for="content" class="form-label form-required">Content</label>
                <textarea class="form-control" id="content" name="content" placeholder="Content" data-editor="{{ json_encode(['width' => '99%', 'height' => '425px']) }}">{{ old('content', $postInfo->content) }}</textarea>
            ...
            <div class="form-row">
                <div class="col-12 col-md-6 form-group">
                    ...
                    <div data-for="category_id" class="list-folder bdr-c3 p05 r05">
                        @include('backend.post.partials.list_category', ['arrListCategory' => $listCategory, 'arrListOn' => old('category_liston', $postInfo->category_liston), 'selected' => old('category_id', $postInfo->category_id)])
                    </div>
                        ...
                        <div class="col-6 form-group">
                            ...
                            <select class="form-control" id="priority" name="priority">
                                @foreach (config('constants.post.priority') as $name => $value)
                                    <option value="{{ $value }}" {!! old('priority', $postInfo->priority) == $value ? ' selected="selected"' : '' !!}>{{ ucfirst($name) }}</option>
                                @endforeach
                            </select>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="seo_title" name="seo_title" value="{{ old('seo_title', $postInfo->seo_title) }}" placeholder="Web title">
                        </div>
                        <div class="form-group">
                            <label for="seo_keywords" class="form-label">Keywords</label>
                            <input type="text" class="form-control" id="seo_keywords" name="seo_keywords" value="{{ old('seo_keywords', $postInfo->seo_keywords) }}" placeholder="Keywords">
                        </div>
                        <div class="form-group">
                            <label for="seo_description" class="form-label">Description</label>
                            <textarea class="form-control" rows="3" id="seo_description" name="seo_description" placeholder="Description">{{ old('seo_description', $postInfo->seo_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
