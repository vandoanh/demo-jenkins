...
<article class="container c-mt-3">
    <h2 class="mb-4">Danh mục: {{ $categoryInfo->title }}</h2>
    @if ($listPost->isNotEmpty())
        <div class="card-list d-flex justify-content-between flex-wrap">
            @foreach ($listPost as $post)
                <div class="card">
                    <a href="{!! route('post.detail', [$post->code, $post->id]) !!}" title="{{ $post->title }}"><img class="card-img-top" src="{!! image_url($post->thumbnail_url, '348x261') !!}" alt="{{ $post->title }}"></a>
                    <div class="card-body">
